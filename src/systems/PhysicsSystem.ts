import {
  ...
  TERMINAL_VELOCITY, MAX_UPWARD_VELOCITY, CEILING_Y, GROUND_Y
} from '../config'
...
export class PhysicsSystem {
  ...
  update(dt: number, isHolding: boolean, windForce: number, liftMult = 1, gravityMult = 1): number {
    ...
    this.velocityY = clamp(this.velocityY, MAX_UPWARD_VELOCITY, TERMINAL_VELOCITY)

    return this.velocityY * dt
  }

  clampY(y: number): number {
    return clamp(y, CEILING_Y, GROUND_Y)
  }
